...
//    qDebug()<< "rawMouseEvent:" << dx << dy << buttons;
}

void InputConvertGame::mouseMove(int dx, int dy) {

//    int interval = m_ctrlMouseMove.elapsedTimer.elapsed();
//    if (interval > 10) {
//        qDebug()<< "rawMouseEvent interval:" << interval;
//    }
//    m_ctrlMouseMove.elapsedTimer.restart();
    ...
    double scaleX = dx / speedRatio.x();

    double boundaryRight = (1 - m_ctrlMouseMove.maxBoundary) * m_frameSize.width();

    if (scaleX > boundaryRight * 0.75) {
        qDebug()<< "reach right boundary" << scaleX << boundaryRight;
    }

    double x = scaleX / m_frameSize.width();


    QPointF currentConvertPos(
            m_ctrlMouseMove.lastConvertPos.x() + x,
...
//        qDebug() << "over the boundary";
        ...
        if (currentConvertPos.x()>= 1) currentConvertPos.setX(0.99);
...
//        qDebug() << "steer wheel touch up end"<< id;
        return;
    }
    if (!m_ctrlSteerWheel.delayData.queuePos.empty()) {
...
//            qDebug() << "steer move center";
            ...
            getDelayQueue(
                    ...
                    8, 0, 15, m_ctrlSteerWheel.delayData.path);
...
//        qDebug() << "customMouseClick "<<from->button();
        if (from->button()==Qt::LeftButton) {
            ...
            if (from->modifiers() & Qt::ControlModifier) {
                ...
                int delay = QRandomGenerator::global()->bounded(30, 50);
...
void InputConvertGame::stopMobaWheel(int delay) const {

    if (m_ctrlSteerWheel.delayData.timer->isActive()) {
        m_ctrlSteerWheel.delayData.timer->stop();
    }
    if (getTouchID(m_ctrlSteerWheel.touchKey) != -1) {
        m_ctrlSteerWheel.mobaWheel.stopTimer->start(delay);
    }
}
...
bool InputConvertGame::processMobaMouseMove(const QMouseEvent *from) {
    ...
    if (m_ctrlSteerWheel.mobaWheel.skillPressed && !m_ctrlSteerWheel.mobaWheel.quickCast) {
        ...
        double perspectiveScale = 0.6 * rawDistance.y() + 1;
...
void InputConvertGame::processMobaSkill(const KeyMap::KeyMapNode &node, const QKeyEvent *from) {
    ...
    if (QEvent::KeyPress == from->type()) {
        ...
        int id = attachTouchID(from->key());
        QPointF clickPos = node.data.mobaSkill.keyNode.pos;
